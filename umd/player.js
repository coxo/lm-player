(function (global, factory) {
  ...
  function PlayerBar({
    ...
    const [dep, setDep] = React.useState(Date.now()); // useEffect(() => {

    const fullscreen = React.useCallback(() => {
      // !isFullscreen(playContainer) ? api.requestFullScreen() : api.cancelFullScreen()
      setDep(Date.now());
    }, [api, playContainer]);
    const setScale = React.useCallback((...args) => {// const dragDom = playContainer.querySelector('.player-mask-layout')
      // api.setScale(...args)
      // let position = computedBound(dragDom, api.getPosition(), api.getScale())
      // if (position) {
      //   api.setPosition(position, true)
  ...
  function SinglePlayer({
    ...
    const [isH265, setH265] = React.useState(false);
